...
function init() {
  ...
  // create forms for each element with the class custom-form
  const forms = document.querySelectorAll(".custom-form");
  forms.forEach((form) => new CustomForm(form));

  // if the user is not logged in, redirect to the home page
  if (document.body.dataset.auth === "true" && !auth.isLogged()) {
    alert("You are not logged in. Redirecting to the home page.");
    window.location.href = "./Home.html";
  }
  if (document.getElementById("emailAddress")) {
    const emailAddress = auth.getCurrentUserEmail();
    changeUserContent(emailAddress);
  }
}
...
// ----- Element height calculation for 'elementVariables' array -----
